...
for list_index in range(len(ra)):
    ...
    flux_dict = vh.get_multiband_flux_from_circ_ap(cutout_pos=cutout_pos,
                                                   hst_list=hst_file_name_list, hst_err_list=hst_err_files,
                                                   hst_ap_rad_list=[ap_list[4]]*len(hst_file_name_list),
                                                   nircam_list=nircam_file_name_list, #nircam_err_list=nircam_err_files,
                                                   nircam_ap_rad_list=[ap_list[4]]*len(nircam_file_name_list),
                                                   miri_list=miri_file_name_list, miri_err_list=miri_err_file_name_list,
                                                   miri_ap_rad_list=[ap_list[4]]*len(miri_file_name_list),
                                                   hst_channel_list=hst_channel_list, nircam_channel_list=nircam_channel_list, miri_channel_list=miri_channel_list,
                                        ...
                                        re_center_peak_hst=False, re_center_peak_nircam=False, re_center_peak_miri=False,
                                        hdu_number_hst=0, hdu_number_nircam='SCI', hdu_number_miri=0,
                                        hdu_err_number_hst=0, hdu_err_number_nircam='ERR', hdu_err_number_miri=0,
                                        flux_unit='mJy')
    ...
    np.save('data_output/result_dict_%i_new' % list_index, result_dict)
...
flux_file = open("silicat_flux_file_new.dat", "w")
